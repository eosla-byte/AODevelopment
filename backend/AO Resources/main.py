...
from database import (
    get_projects, get_project_details, create_project, update_project_meta, 
    get_collaborators, get_collaborator_details, create_collaborator, 
    update_collaborator, update_collaborator_picture, toggle_archive_collaborator,
    add_adjustment, remove_adjustment, get_months_worked,
    get_expenses_monthly, add_expense_column, add_expense_card, 
    copy_expense_card, delete_expense_card, delete_expense_column,
    update_expense_card_files, get_or_create_default_column,
    get_total_collaborator_allocations, get_project_stats_by_category,
    get_users, get_user_by_email, save_user, delete_user, User,
    start_revit_session, heartbeat_session,
...
async def admin_plugin_users_page(request: Request):
    ...
    for u in users:
        ...
        user_sessions = [s for s in sessions if s.get("user_email") == u.email]
        if user_sessions:
            # Sort by last heartbeat descending
            user_sessions.sort(key=lambda x: x.get("last_heartbeat", ""), reverse=True)
            last_session = user_sessions[0]
            
            try:
                lh = datetime.datetime.fromisoformat(last_session.get("last_heartbeat"))
                # If last heartbeat was < 5 minutes ago = ONLINE
                diff = (datetime.datetime.now() - lh).total_seconds() / 60.0
                if diff < 5 and last_session.get("is_active", False):
                    is_online = True
...
async def update_file_metadata(project_id: str, category: str = Form(...), filename: str = Form(...), amount: str = Form(...), note: str = Form("")):
    try:
        val = float(amount)
...
async def read_hr_plugin_logs(request: Request, collab_id: str):
    ...
    # Flatten sessions
    sessions = get_plugin_sessions()
    user_sessions = [s for s in sessions if s.get("user_email") == collab.email]
    
    # Sort DESC
    user_sessions.sort(key=lambda x: x.get("start_time", ""), reverse=True)
    
    session_logs = []
    for s in user_sessions[:50]: # Last 50 only
        # Calc session stats
        active = 0.0
        idle = 0.0
        files = set()
        ...
        # Try to find meaningful data in logs
        for l in s.get("activity_logs", []):
            active += l.get("active_minutes", 0)
            idle += l.get("idle_minutes", 0)
            if l.get("file_name"): files.add(l.get("file_name"))
            
            # Capture last known revit user / acc project
            if l.get("revit_user"): revit_user = l.get("revit_user")
            if l.get("acc_project"): acc_project = l.get("acc_project")
            
        # Format Times
        try:
            st = datetime.datetime.fromisoformat(s.get("start_time"))
            et = datetime.datetime.fromisoformat(s.get("last_heartbeat"))
            start_fmt = st.strftime("%H:%M")
            end_fmt = et.strftime("%H:%M")
            date_fmt = st.strftime("%Y-%m-%d")
            day_name = st.strftime("%A")
        except:
            start_fmt = "--:--"
            end_fmt = "--:--"
            date_fmt = s.get("start_time", "")[:10]
            day_name = ""
        ...
        session_logs.append({
            ...
            "end_time": end_fmt,
            "active_mins": round(active, 1),
            "idle_mins": round(idle, 1),
            "files": list(files),
            "machine": s.get("machine_name", "Unknown"),
            "ip_address": s.get("ip_address", "--"),
            "sync_count": len(s.get("sync_logs", [])),
...
async def api_assign_project(req: AssignProjectRequest):
    # root_path = get_root_path() # Removed
    users = get_users()
    target_user = None
    existing_idx = -1
    ...
    target_user.assigned_projects = req.project_ids
    target_user.permissions = req.permissions
    
    # Save manually using save_user Logic
    # We can just call save_user if update dict was better, but let's re-save
    # Re-saving modifies the list in memory inside the function, need to be careful.
    # Actually save_user handles it by ID.
    
    save_user(target_user)
    return {"status": "success", "user": target_user.__dict__}
...
async def api_client_dashboard(request: Request):
    ...
    # root_path = get_root_path() # Removed
    user = get_user_by_email(root_path, email)
